...
const Login = () => {
  ...
  const { login, loading, isAuth, user } = useAuthStore();
  const navigate = useNavigate();

  useEffect(() => {
    if (isAuth && user?.role) {
      const { getRoleBasedRoute } = useAuthStore.getState();
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Or{' '}
            <Link to="/register" className="font-medium text-accent hover:text-yellow-600">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600">
              Test accounts: admin@cinema.com, owner@cinema.com, cashier@cinema.com, budi@example.com (password: password)
            </p>
          </div>
        </form>
      </div>
